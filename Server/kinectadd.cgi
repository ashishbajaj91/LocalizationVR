...
def GetXZValues():
	...
	if(IsWebUser()==True):
		...
		if(CheckIfRequestIsGet() == True):
			xvalues = GetX(url)
			zvalues = GetZ(url)
			if (xvalues != None and len(xvalues) > 0 and zvalues != None and len(zvalues) > 0):
				value = [xvalues[0], zvalues[0]]
	return value
	
def GetX(url):
	return extractvalueforkeyfromurl(url,'x')

def GetZ(url):
	return extractvalueforkeyfromurl(url,'z')
